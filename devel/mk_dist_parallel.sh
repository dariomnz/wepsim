...
# ----- Block A: hardware model + software model + core (simulation ctrl + UI) -----
{
  set -e
...
# ----- Block B: WepSIM internalization (i18n) -----
{
  set -e
  echo "  * ws_dist/help/..."
...
# ----- Block D: WepSIM web engine -----
{
  set -e
  echo "  * ws_dist/min.wepsim_webui.js"
...
# ----- Block H: jq merges for default examples -----
{
  set -e
  # MIPS
...
for pid in $PARALLEL_PIDS; do
  if ! wait "$pid"; then
    echo "ERROR: A parallel step failed"
    exit 1
  fi
done
...
# ----- Phase 3a: run remaining steps in parallel -----
FINAL_PIDS=""
...
# Final web bundle
{
  ...
  rm -f ws_dist/min.wepsim_webui.js
} &
FINAL_PIDS="${FINAL_PIDS}${FINAL_PIDS:+ }$!"
...
# ----- Phase 3b: export hardware (needs node + repo) -----
{
  set -e
  echo "  * hw_def for each processor"
  ...
  ./ws_dist/wepsim.sh -a export-hardware -m rvpipe  > ws_dist/repo/hardware/rvpipe/hw_def.json
} &
FINAL_PIDS="${FINAL_PIDS}${FINAL_PIDS:+ }$!"

# Wait for all Phase 3 steps
echo "  Waiting for final steps..."
for pid in $FINAL_PIDS; do
  if ! wait "$pid"; then
    echo "ERROR: A final step failed"
    exit 1
  fi
done
echo ""
